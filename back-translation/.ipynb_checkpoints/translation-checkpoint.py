...
if os.path.exists(dir+'/trg.field'):
    ...
    TRG = Field(tokenize = tokenize_en,
                ...
                fix_length=field_fix_length)
#######################################################################################################################
# def save_data(data, name, savedir=dir):
#     save_path = os.path.join(savedir, f"{name}")
...
# valid_data = None
# test_data = None
# if os.path.exists(dir+'/train_data'):
#     train_data = load_data(os.path.join(dir, 'train_data'))
# if os.path.exists(dir+'/valid_data'):
#     valid_data = load_data(os.path.join(dir, 'valid_data'))
# if os.path.exists(dir+'/test_data'):
#     test_data = load_data(os.path.join(dir, 'test_data'))
# else:
#     train_data, valid_data, test_data = WMT14.splits(exts=('.en', '.de'), fields=(SRC, TRG))
#     save_data(train_data, "train_data", savedir=dir)
#     save_data(valid_data, "valid_data", savedir=dir)
#     save_data(test_data, "test_data", savedir=dir)
...
if os.path.exists(dir+'/src.field') and os.path.exists(dir+'/src.vocab'):
    ...
    SRC.build_vocab(train_data, min_freq=vocab_min_freq, max_size=vocab_max_size, specials=['<style>'])
    save_field(SRC, "src", dir)
    save_vocab(SRC, "src", dir)
if os.path.exists(dir+'/trg.field') and os.path.exists(dir+'/trg.vocab'):
    pass
else:
    TRG.build_vocab(train_data, min_freq=vocab_min_freq, max_size=vocab_max_size)
...
def calculate_output_loss(epoch, model, iterator, criterion, is_eval, optimizer=None, clip=None):
...
def train(epoch, model, iterator, criterion, is_eval, optimizer=None, clip=None):
    if is_eval == False:
        model.train()
        epoch_translation_loss = calculate_output_loss(epoch, model, iterator, criterion, False, optimizer, clip)
    else:
        model.eval()
        with torch.no_grad():
            epoch_translation_loss = calculate_output_loss(epoch, model, iterator, criterion, True)
...
if is_only_evaluation == False:
    ...
    for epoch in range(N_EPOCHS):
        ...
        train_translation_loss = train(epoch, model, train_iterator, criterion, False, optimizer, CLIP)
        valid_translation_loss = train(epoch, model, valid_iterator, criterion, True)
        ...
        if another_early_stop_cnt == another_early_stop_lookout:
            ...
            break
#######################################################################################################################
checkpoint = torch.load(dir+'/checkpoint.pt')
model.load_state_dict(checkpoint['state_dict'])
#optimizer.load_state_dict(checkpoint['optimizer'])
#epoch = checkpoint['epoch']

test_loss = train(None, model, test_iterator, criterion, True)
print(f'| Test Loss: {test_loss:.7f} | Test PPL: {math.exp(test_loss):7.3f} |', flush=True)
#######################################################################################################################
...
#def calculate_bleu(data, src_field, trg_field, model, device, max_len):
#    trgs = []
#    masked_trgs = []
#
#    pred_trgs = []
...
## ######################################################################################################################
#b_score, masked_b_score = calculate_bleu(test_data, SRC, TRG, model, device, 50)
#print('\n')
#print(f'BLEU score on test data = {b_score*100:.5f}', flush=True)
#print(f'BLEU score on test data = {masked_b_score*100:.5f}', flush=True)
#print('\n')
#####################################################################################################################
def print_output(data, length, file_name):
    with open(file_name, 'w') as fileo:
        for idx in range(length):
            ...
            predicted_trg_trn, attention = translate_sentence(src, SRC, TRG, model, device, 50)

            # print(f'src = {post_processing(src)}', flush=True)
            # print(f'trg = {post_processing(trg)}', flush=True)
            # print(f'translated predicted trg = {post_processing(predicted_trg_trn)}', flush=True)
            fileo.write(post_processing(predicted_trg_trn)+'\n')
            #print('\n', flush=True)
########################################################################################################################
print("Testing data", flush=True)
print('##############################################################################################################')
print_output(train_neg_data, 6400, '../yelp_data/neg/train.de')
print_output(train_pos_data, 6400, '../yelp_data/pos/train.de')
print_output(valid_neg_data, 1000, '../yelp_data/neg/valid.de')
print_output(valid_pos_data, 1000, '../yelp_data/pos/valid.de')
print_output(test_neg_data, 1000, '../yelp_data/neg/test.de')
print_output(test_pos_data, 1000, '../yelp_data/pos/test.de')
##########################################################################################################################
...
#
#
#         predicted_trg_st, attention = translate_sentence(src, SRC, TRG, model, device, 50)
#         print(f'translated predicted trg = {post_processing(predicted_trg_st)}', flush=True)
...
#         similarity_score_masked = snt_ev.similarity(snt_ev.mask_polarity(post_processing(trg)),
#                                                     snt_ev.mask_polarity(post_processing(predicted_trg_st)))
...
#
#     return lm_scores_mean, similarity_scores_mean, masked_similarity_scores_mean
# ########################################################################################################################
# print("Testing data", flush=True)
# print('##############################################################################################################')
# lm_scores_mean, similarity_scores_mean, masked_similarity_scores_mean = evaluation(test_data)
